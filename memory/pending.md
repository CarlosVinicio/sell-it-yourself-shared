...
| T-115 | Construir T4: Checklist documentación por CCAA (función pura + 18 tests + Form/Result/Calculator con persistencia localStorage + Zod enums + HowTo+FAQPage JSON-LD + email capture T4 + 2 cross-links) | 2026-05-19 | Commits 9777932 (blog→guides) + 9c55b14 (T4): 19 CCAA, 18 documentos, lógica condicional (cédula 8 regiones, ITE >50 años, hipoteca). UI: select nativo + InputCheckbox, ResultCard+Accordion, localStorage. Build ✅ (6.49 kB) Lint ✅ Tests 58 totales (18 nuevas) ✅. 🔴 ccaa-data verificado: false (T-311). |
| Bugfix-Routing | ✅ Arreglar routing /guias: renombrar directorio de `guides` → `guias` para cumplir D-010 (rutas en español) | 2026-05-19 | Commit 03cf843: El directorio de ruta estaba mal nombrado en inglés (/guides) en lugar de español (/guias), causando 404s en dev y prod. Renombrado src/app/(marketing)/{guides → guias}/ manteniendo src/features/guides/ en inglés per D-008. Routes /guias y /guias/[slug] ahora funcionan ✅. |
